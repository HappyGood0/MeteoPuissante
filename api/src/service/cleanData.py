...
def modify_data(data):	
	...
	second_lighning = data[data["nst_of_storm"] == 2].copy()
	second_lighning["original_index"] = second_lighning.index -1
	"""data.insert(16, "dist_derivative", (second_lighning["dist"] - data.loc[second_lighning["original_index"], "dist"].values) / ((second_lighning["date"] -  data.loc[second_lighning["original_index"], "date"].values) / pd.Timedelta(seconds=1)))
	data.insert(17, "cluster_dist", data.groupby('airport', group_keys=False)[['date', 'dist']]\
							.rolling(5, closed = "right", min_periods=1)\
							.rolling("1h", on="date", closed = "right", min_periods=1)\
							.mean()\
							.reset_index(level=0, drop=True)['dist'])"""
